...
async def stream(client, m: Message):
    ...
    if not media and not ' ' in m.text:
        await m.reply_text("❗ __Kirimi Saya Tautan live streaming / Tautan Video YouTube / Balas Video Untuk Memulai Streaming Video!__")

    elif ' ' in m.text:
        text = m.text.split(' ', 1)
        query = text[1]
        msg = await m.reply_text("🔄 `Sedang Memproses....` ")
        ...
        if match:
            await msg.edit("🔄 `Menghubungkan ke obrolan suara`")
            try:
            ...
            if group_call.is_connected:
                try:
                    ...
                    await msg.edit(f"▶️ **Memulai [Streaming Youtube]({query})!**", disable_web_page_preview=True)
                except Exception as e:
            ...
            await msg.edit("🔄 `Memulai Streaming Langsung ...`")
            livestream = query
            await sleep(2)
            group_call = group_call_factory.get_group_call()
            if group_call.is_connected:
                try:
                    ...
                    VIDEO_CALL[CHAT_ID] = group_call
                    await msg.edit(f"▶️ **Memulai [Streaming Langsung]({query})!**", disable_web_page_preview=True)
                except Exception as e:
                    await msg.edit(f"❌ **Terjadi Kesalahan!** \n\nError: `{e}`")
            else:
                try:
        ...
        if group_call.is_connected:
            try:
                ...
                VIDEO_CALL[CHAT_ID] = group_call
                await msg.edit(f"▶️ **Memulai [Video Streaming](https://t.me/AsmSafone)!**", disable_web_page_preview=True)
            except Exception as e:
                await msg.edit(f"❌ **Terjadi Kesalahan!** \n\nError: `{e}`")
        else:
            try:
        ...
        await m.reply_text("❗ __Kirim Saya Tautan Streaming Langsung / Tautan Video YouTube / Balas Ke Video Untuk Memulai Streaming Video!__")
        return
...
async def endstream(client, m: Message):
    ...
    if CHAT_ID in RADIO_CALL:
        await RADIO_CALL[CHAT_ID].stop()
        RADIO_CALL.pop(CHAT_ID)
        await msg.edit("🔴 **Userbot Telah Terputus Dari Obrolan Suara!**")
        ...
        await msg.edit("🤖 **Silakan Mulai Streaming Terlebih Dahulu!**")
...
async def not_chat(_, m: Message):
    ...
    await m.reply_text(text="**Maaf, Anda Tidak Dapat Menggunakan Bot Ini Di Grup Ini ️! Tapi Anda Bisa Membuat Bot Sendiri Seperti Ini Dari [Source Code](https://Xhamster.com) **", reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
